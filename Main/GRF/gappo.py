...
class IndependentActorHead(nn.Module):
    ...
    def forward(self, obs):
        ...
        if obs.ndim == 2:  # [N, obs_dim]
            ...
            add_batch = True 
        elif obs.ndim == 4:  # [B, T, N, obs_dim]
            E, B, N, obs_dim = obs.shape
            obs = obs.view(E * B, N, obs_dim)  # [B*T, N, obs_dim]
...
class GAPPO(Train):
    def __init__(self, env, args):
        ...
        if args.shared_backbone:
            ...
            backbones = nn.ModuleList([
                Init_GAPPO(
                    algorithm=args.algorithm,
                    actions=19,
                    n_agents=self.args.n_agents,              # each backbone sees only its own obs
                    input_features=env.observation_spec["player", "observation"].shape[-1],
                    device=args.device,
                    global_step=self.global_step,
                    args=args,
        ...
        self.policy = ProbabilisticActor(
            ...
            log_prob_key=("player", "sample_log_prob")
        )
        ...
        self.loss_module.set_keys(
            ...
            sample_log_prob=("player", "sample_log_prob"),  
            value_target=("player", "value_target"),  
        ...
        self.optimizer = torch.optim.Adam(self.loss_module.parameters(), args.learning_rate, fused=True if torch.cuda.is_available() else False)
        ...
        super().__init__(args, env, self.collector, self.replay_buffer, self.loss_module, self.optimizer)